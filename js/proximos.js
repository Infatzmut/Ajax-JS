  ...
  //Ordena los eventos segun la fecha (los mas cercanos primero)
    eventos.forEach(element=>{
      if(element.fecha >= fecAct )
        proximos.push(element.fecha) 
       })
    proximos.sort();//orden fechas
    //Crea un string que contenga el HTML que describe el detalle del evento
  //Recorre el arreglo y concatena el HTML para cada evento
  //Modifica el DOM agregando el html generado dentro del div con id=evento
      for(var i=0;i<proximos.length;i++){
         var evento = eventos.find(element =>element.fecha ==proximos[i])
        ...
        div.innerHTML=texto
        document.getElementById("proximos").appendChild(div)
      }
  })
